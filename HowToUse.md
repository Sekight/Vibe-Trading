...
### 8.45 WebUI「持仓与风险」tab 的两张图是怎么算的？（每日组合持仓 / 每天账户风险度）

- 位置：报告页「分析图」右边、「分析」左边的「持仓与风险」tab，含两张图。数据源都是 `artifacts/positions.csv`（引擎每 bar 每个 code 的目标权重）+ `artifacts/equity.csv`（每 bar 总权益）。
...
- **多标的是怎么看持仓与风险**（同一账户同时交易多个标的，如某天 RB 多 + TA 空 + FG 多）：
  ...
  - **看哪个标的贡献最大 → 组合级图看不到**（只能看总数）——这是二期可迭代点（逐标的 + 下拉框按需加载），多标的策略上线前需要它；
  - 图 1 三线是同刻快照、图 2 峰值可能来自当天不同 bar——不要把"图 1 的毛"和"图 2 的峰值"当成同一时刻的全景。
- **必须知道的模型口径差异**：
