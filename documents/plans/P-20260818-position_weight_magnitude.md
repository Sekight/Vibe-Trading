# 计划：组合/单票仓位指标改为"不关乎多空"的毛/单边口径 + 多标的账户观察支持
...
- 现状口径（`agent/backtest/engines/base.py` metrics 段）：`avg_portfolio_weight` = `target_pos.sum(axis=1).mean()`、`max_portfolio_weight` = `.max()`、`max_single_weight`（V033 后）= 按 `weight_groups` 分组带符号求和后 `.max()`——三者都是**净敞口（带符号）**口径；且 `max` 聚合会丢弃空头日（负值）→ **最大类指标只反映多头侧峰值**（v437fixB 实测：净敞口最大正值 20.97%，空头侧 -41.28% 取不到）。
...
- 多标的场景口径行为（关键分析，用户提出 RB 多 + TA 空 + FG 多）：
  ...
  - **单边**（按 `weight_groups` 组内取 max(多头和,|空头和|) 跨组合计）= 各标的各自的大边之和 → **真实保证金占用**（按期货"同一合约对锁单边收保证金"）。**多标的且各标的方向独立时，单边 = 毛**（组间不互抵）；两者只在"同一标的内多空并存（锁仓）"时不同（单边 < 毛）；
  - **净敞口** = `sum w` = 多空相抵后的**市场风险净暴露**（RB +0.1 + TA -0.08 + FG +0.12 → +0.14）。
- 用户观察目标（不关乎多空）：①平均每天资金利用率 = avg 毛持仓；②持仓最大时用了多少仓位 = max 毛持仓；③哪个单票仓位最大 = 单票（按 weight_groups 分组）单边权重峰值。三者恰好都是**毛/单边口径**，与 V034 图 1 一致，与现有净敞口指标区不同。
...
- 做什么：让指标区（`avg_portfolio_weight` / `max_portfolio_weight` / `max_single_weight`）能回答用户三个资金管理问题，**口径从"净敞口"改为"毛/单边（不关乎多空）"**：
  ...
  3. `max_single_weight` → 最大单票仓位 = 按 weight_groups 分组后各组单边权重的峰值（多空都算），**并指出是哪个标的**（新增 companion 字段或指标）。
- 范围 / 边界：
...
- 验收标准（待拍板后细化）：
  ...
  - 无声明 weight_groups 的单标 run：单票口径退回单 code 毛口径；
  - 旧指标字段兼容（旧 run 读旧值不崩）。
...
1. **base.py metrics 段**：
   ...
   - `max_single_weight` = 按 `weight_groups` 分组后各组单边（max(多头和,|空头和|)）逐 bar 求值，再取全局峰值；新增 `max_single_weight_code`（峰值对应标的/组名）→ 落到 metrics/run_card/digest。
...
5. **测试**：指标口径断言（多空日、多标的合成 run、无 weight_groups 回退、旧 run 兼容）。
...
- 单边口径依赖 weight_groups 声明：无声明时单票 = 单 code 毛口径（V033 已定义回退）。
