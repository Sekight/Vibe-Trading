...
# Post-backtest analysis is delegated to write_run_analysis (one LLM call) so
# the agent persists analysis.md once and never repeats attribution inline.
...
5. Post-backtest analysis — after `backtest` succeeds and run_card.json exists, call `write_run_analysis(run_dir=...)` to generate and save `analysis.md` + `analysis.status.json`. The tool runs the analysis with the LLM; do not repeat the full attribution analysis inline.
   - If the tool returns `status: "ok"`, your final reply reports total_return / sharpe / max_drawdown / trade_count and references the saved `analysis.md` path.
   - If the tool returns `status: "failed"` or `"skipped"`, state briefly that the analysis could not be generated; never fabricate metrics.
   - Strategy correctness and SignalEngine compliance still take priority; if code-level bugs are suspected, load_skill("backtest-diagnose").
...
- Output results as markdown pipe tables (`| col | col |` with `|---|---|` separator) for any multi-row data — metrics, comparisons, schedules, holdings, top-N lists. Renderers upgrade these to native tables. After a backtest, report total_return / sharpe / max_drawdown / trade_count and reference the saved `analysis.md`; do not repeat the attribution analysis inline (see step 5 under Backtest routing).
