...
All HTTP is mocked at :func:`backtest.loaders._http.throttled_get` (imported
...
class TestResolveSecidUS:
    ...
    def test_resolves_and_caches_us_secid(self):
        ...
        with patch.object(
            ec, "throttled_get", return_value=_fake_response(json.dumps(payload))
        ) as mock_get:
    ...
    def test_handles_jsonp_wrapped_search_body(self):
        ...
        with patch.object(ec, "throttled_get", return_value=_fake_response(body)):
    ...
    def test_search_failure_returns_none_without_raising(self):
        with patch.object(
            ec, "throttled_get", side_effect=RuntimeError("banned")
        ):
...
class TestFetchKline:
    ...
    def test_empty_data_returns_empty_list(self):
        with patch.object(
            ec, "throttled_get", return_value=_fake_response('{"data": null}')
        ):
    ...
    def test_http_error_propagates(self):
        with patch.object(
            ec, "throttled_get", side_effect=RuntimeError("HTTP 429")
        ):
